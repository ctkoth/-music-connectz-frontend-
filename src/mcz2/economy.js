...
// Free 10% · Premium 5% · StatZ 2%. All transactions surface this transparently.

export function devTaxFor(tier) {
  const t = (tier || "").toLowerCase();
  if (t.includes("statz") || t.includes("stats")) return { label: "StatZ", rate: 0.02 };
  if (t.includes("premium") || t.includes("pro")) return { label: "Premium", rate: 0.05 };
  return { label: "Free", rate: 0.10 };
}
